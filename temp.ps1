...
if($AV2.ExitCode -eq 0 -or $AV2.ExitCode -eq 3010){Write-Host "半透明mcafee安装成功" -ForegroundColor Green}
else{Write-Host "半透明mcafee安装失败" -ForegroundColor Red}
}
...
Start-Sleep -Seconds 120
if(Get-Item "C:\rsprinterex\PrintToCloud.exe" -ErrorAction SilentlyContinue){Write-Host "刷卡打印插件安装成功" -ForegroundColor Green}
else{Write-Host "刷卡打印插件安装失败" -ForegroundColor Red}
}
...
Start-Sleep -Seconds 120
if(Get-Process zbox_client -ErrorAction SilentlyContinue){Write-Host "企业网盘安装成功" -ForegroundColor Green}
else{Write-Host "企业网盘安装失败" -ForegroundColor Red}
}
